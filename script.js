document.addEventListener('DOMContentLoaded', () => {
    ...
    // Helper function to apply necessary game permissions to iframe elements
    const configureGameIframe = (iframe) => {
        if (!iframe) return;
        iframe.setAttribute('allow', 'autoplay; fullscreen; gamepad; pointer-lock; accelerometer; gyroscope; microphone; camera; clipboard-write; focus-without-user-activation');
    ...
    // Ensure the main static iframe has standard game permissions set
    if (gameFrame) {
        configureGameIframe(gameFrame);
    }
            ...
            if (progress >= 100) {
                ...
                if (localStorage.getItem('hub_cloak') === 'true') {
                    ...
                    if (win) {
                        ...
                        win.document.body.innerHTML = `<iframe src="${url}" style="width:100%; height:100%; border:none;" allow="autoplay; fullscreen; gamepad; pointer-lock; accelerometer; gyroscope;" allowfullscreen="true"></iframe>`;
    ...
    // Prevent clicks on inputs inside the modal from triggering backdrop closing
    if (avatarUpload) {
    ...
    // Updated backdrop click detector
    if (modal) {
        modal.addEventListener('click', (e) => {
            // Only close if the background backdrop itself was directly clicked
